...
The settings window opens on **Overview**, then **Devices**, **General**, **Notifications**, **Maintenance**, and **About**. Use **Devices > Output** and **Devices > Input** to choose optional preferred devices for Playnite startup and to customize device names, icons, visibility, and preferred volume. Choosing **Keep the Windows default** leaves that direction untouched, including when switching between Playnite Desktop and Fullscreen. Overview, Fullscreen, and the Desktop indicator also follow Windows endpoint add/remove/default changes live; volume-meter property noise is ignored. Each listed endpoint shows its Windows state and battery as Overview-style badges; the battery uses an em dash when Windows does not report a value, so `0%` always means a real empty-battery reading. Endpoints disabled in Windows Sound settings are hidden by default, with a separate Output and Input option to show them again. Stale disconnected endpoints stay hidden unless the user customized them or a game profile still references them. Game profiles can temporarily override either preferred device and restore the previous output/input after the game closes. **General > Game profiles** includes a central manager for reviewing, editing, and removing every saved output, input, Spatial Sound, and game-volume assignment.

Under **General > Desktop**, Audio Switcher can stay available in Playnite's Desktop top bar with a fixed icon or the icon of the active output device. An optional semantic battery color applies the same red, orange, yellow, or green state to every visible part of the top-bar indicator. Under **General > Battery**, Desktop can show the icon alone, the icon plus a reported percentage, or replace the icon with the percentage while automatically restoring the icon when no battery value is available. Fixed-width circular top-panel buttons automatically use a compact presentation that always shows the icon and hides the potentially ambiguous standalone percentage; the icon still uses the battery color when available. Hovering shows `Device name: 68%` when available, and clicking opens Audio Switcher settings. The Fullscreen `BatteryWidget` has its own independent display mode and icon. Notifications are available from their own top-level tab. Maintenance tools and experimental Spatial Sound setup live under **Maintenance** and **General > Spatial Sound**.
