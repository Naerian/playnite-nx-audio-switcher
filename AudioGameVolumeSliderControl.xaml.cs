...
namespace PlayniteAudioSwitcher
{
    public partial class AudioGameVolumeSliderControl : PluginUserControl
    {
        ...
        public AudioGameVolumeSliderControl(AudioSwitcherPlugin plugin)
        {
            this.plugin = plugin;
            InitializeComponent();
            Loaded += AudioGameVolumeSliderControl_Loaded;
        }

        private void AudioGameVolumeSliderControl_Loaded(object sender, RoutedEventArgs e)
        {
            Refresh();
        }
        ...
        private void SetSliderValue(int value, bool muted, bool isAvailable)
        {
            ...
            try
            {
                ...
                VolumeLabel.Text = isAvailable
                    ? muted ? plugin.Loc("LOCAS_GameMuted") : $"{(int)Math.Round(VolumeSlider.Value)}%"
